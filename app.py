...
def build_player_image(item: object) -> str:
    ...
    if(item['imageUrl'] != ''):
        ...
        if response.status_code != 404:
            return f'<img src="{item['imageUrl']}" alt="{item['name']}" style="width: 120px; height: 180px;">'
        else:
            # see if we have the image locally if we 404
            if(os.path.exists(f"{player_image_dir}{item["playerID"]}.jpg")):
                return f'<img src="{player_image_dir}{item["playerID"]}.jpg" alt="{item['name']}" style="width: 120px; height: 180px;">'
    else:
